...
// Spring config to prevent jitter
const SPRING_CONFIG = {
  type: 'spring',
  stiffness: 200,
  damping: 25
...
const ChatWidget = () => {
  ...
  // Focus input after view mode changes
  useEffect(() => {
  ...
  // Get wrapper alignment classes based on view mode
  ...
  // Get card dimensions based on view mode
    ...
    <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100/80 bg-white rounded-t-3xl">
      ...
      <div className="flex items-center gap-0.5">
        ...
        <button
          ...
          className="p-2.5 hover:bg-gray-100 rounded-xl transition-all duration-200"
          aria-label={isDocked ? 'Expand to center' : 'Dock to sidebar'}
          title={isDocked ? 'Expand to center' : 'Dock to sidebar'}
        >
          {isDocked ? (
            <Maximize2 className="w-[18px] h-[18px] text-gray-400" />
          ) : (
            <Minimize2 className="w-[18px] h-[18px] text-gray-400" />
          )}
        </button>
              ...
              <div className="flex flex-wrap gap-2">
                {message.actions.map((action, idx) => (
                  <motion.button
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 border border-[#0176D3]/30 text-[#0176D3] rounded-full hover:bg-blue-50 hover:border-[#0176D3]/50 transition-all duration-200 text-sm font-medium"
                  >
          ...
          return (
            ...
            >
              <div className="bg-gradient-to-br from-[#0176D3] to-[#0B5CAB] rounded-2xl rounded-tr-lg px-4 py-3 max-w-[85%] shadow-lg shadow-blue-500/10">
                <p className="text-[14px] text-white">{message.content}</p>
              </div>
            </motion.div>
          );
        }
  ...
  const ChatInputField = () => (
    <div className="px-4 py-4 border-t border-gray-100/80 bg-white rounded-b-3xl">
      <div className="relative flex items-center bg-gray-50 rounded-2xl border border-gray-200/80">
        <div className="pl-4">
          <BotAvatar size="sm" />
        </div>
        <input
          ...
          className="flex-1 bg-transparent px-4 py-3.5 text-[15px] text-gray-800 placeholder-gray-400 focus:outline-none"
        />
        <motion.button
  ...
  return (
    <div
      ...
      onClick={(e) => {
        // Close to omnibar when clicking backdrop in wide canvas mode
        if (viewMode === VIEW_MODES.WIDE_CANVAS && e.target === e.currentTarget) {
          setViewMode(VIEW_MODES.OMNIBAR);
        }
      }}
    >
      {/* The Chat Card - uses layout prop for smooth animations */}
      <motion.div
        layout
      ...
      >
        ...
        {/* WIDE CANVAS VIEW */}
        {viewMode === VIEW_MODES.WIDE_CANVAS && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
        ...
        {/* DOCKED SIDEBAR VIEW */}
        {viewMode === VIEW_MODES.DOCKED_SIDEBAR && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
